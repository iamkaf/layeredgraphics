...
test("landing page presents the current engine showcase", async ({ page }) => {
  await page.goto("./");
  await expect(page).toHaveTitle(/Layered Graphics/);
  await expect(page.getByRole("heading", { level: 1 })).toContainText("Graphics authoring");
  const showcase = page.getByAltText(/Editable Layered Graphics/);
  await expect(showcase).toBeVisible();
  await expect(showcase).toHaveJSProperty("naturalWidth", 1440);
  await expect(showcase).toHaveJSProperty("naturalHeight", 960);
  await expect(page.getByRole("heading", { name: "Not implemented" })).toBeVisible();
  await expect(page.locator('a[href$="showcase.kgfx"]')).toHaveAttribute("href", "/layeredgraphics/showcase.kgfx");
});
